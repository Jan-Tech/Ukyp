...
function StarRating({ rating, interactive = false, onChange }: { rating: number; interactive?: boolean; onChange?: (r: number) => void }) {
  ...
  return (
    <div className="flex gap-1">
      {[1, 2, 3, 4, 5].map((star) => (
        <button key={star} type={interactive ? "button" : undefined} onClick={interactive ? () => onChange?.(star) : undefined}
          onMouseEnter={interactive ? () => setHovered(star) : undefined} onMouseLeave={interactive ? () => setHovered(0) : undefined}
...
export default function ProfessionalProfilePage() {
  ...
  if (notFound || !professional) return (
    <div className="max-w-4xl mx-auto px-4 py-20 text-center text-gray-400">
      ...
      <p className="text-lg">Hünärmen tapylmady</p>
    </div>
  );
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="flex flex-col sm:flex-row gap-6 items-start">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-3 mb-2">
              ...
              {p.isVerified && <span className="badge-verified">✓ {t.profile.verified}</span>}
              {p.isBoosted && <span className="badge-boosted">⭐ {t.card.boosted}</span>}
            </div>
            <div className="flex items-center gap-3 mb-3 flex-wrap text-sm text-gray-500">
              {avgRating != null && (
                <div className="flex items-center gap-1">
                  <StarRating rating={avgRating} />
                  <span>{avgRating.toFixed(1)} ({p.reviews?.length ?? 0} {t.card.reviews})</span>
                </div>
              )}
              <span>📍 {p.city}</span>
              <span>⏱ {p.yearsExp} {t.profile.years}</span>
              {p.hourlyRate && <span className="font-semibold text-primary">{p.hourlyRate} {t.card.perHour}</span>}
            </div>
            <div className="flex flex-wrap gap-1.5">
              {(locale === "ru" ? (p.skillsRu ?? p.skills) : p.skills)?.map((skill: string) => (
                <span key={skill} className="bg-primary/10 text-primary text-xs font-medium px-2.5 py-1 rounded-full">{skill}</span>
              ))}
            </div>
          </div>
          <div className="flex flex-col gap-2 shrink-0">
            {p.user.phone && <a href={`tel:${p.user.phone}`} className="btn-primary px-5 py-2 rounded-xl text-sm text-center">📞 {t.profile.contact}</a>}
            <a href={`mailto:${p.user.email}`} className="btn-secondary px-5 py-2 rounded-xl text-sm text-center">✉️ Email</a>
          </div>
        </div>
      </div>
      ...
      <div className="flex border-b border-gray-200 mb-6 gap-1">
        {(["about", "portfolio", "reviews"] as const).map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            ...
            {t.profile[tab]}{tab === "reviews" && ` (${p.reviews?.length ?? 0})`}
          </button>
        ))}
      </div>
      ...
      {activeTab === "about" && (
        <div className="card p-6">
          ...
          <p className="text-gray-600 leading-relaxed">{locale === "ru" ? (p.bioRu ?? p.bio) : p.bio}</p>
        </div>
      )}
      ...
      {activeTab === "portfolio" && (
        <div>
          {(!p.portfolio || p.portfolio.length === 0) ? (
            <div className="card p-12 text-center text-gray-400">Portfolio boş</div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {p.portfolio.map((item: any) => (
                <div key={item.id} className="card overflow-hidden">
                  ...
                  <div className="p-4">
                    <h3 className="font-semibold text-sm">{locale === "ru" ? (item.titleRu ?? item.title) : item.title}</h3>
                    <p className="text-xs text-gray-500 mt-1">{locale === "ru" ? (item.descriptionRu ?? item.description) : item.description}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {activeTab === "reviews" && (
        <div className="space-y-4">
          ...
          <ReviewForm t={t} locale={locale} professionalId={id} onReviewAdded={(review) => setProfessional((prev: any) => ({ ...prev, reviews: [review, ...(prev.reviews ?? [])] }))} />
        </div>
      )}
    </div>
  );
